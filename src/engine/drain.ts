  ...
  return {
    ...
    async runBasha(item, ctx) {
      ...
      if (!res.ok) {
        ...
        return { ok: false, branch: ctx.branch, reason: `${why}: ${distillFailure(res.stdout)} (log: ${logRel})` };
      }
      ...
      return { ok: false, branch: ctx.branch, reason: `${blocked ?? "no DONE/BLOCKED marker"} (log: ${logRel})` };
    },
