...
test("Source field round-trips (incl. a value containing colons)", () => {
  const item: QueueItem = {
    ...
    status: "needs-intake",
    origin: "auto",
    source: "todo:tools/orchestrator/cli.ts:ab12cd34",
    goal: "g",
    territory: ["tools/**"],
    doneWhen: "v",
    clarifications: [],
...
test("Open-question field round-trips even when the question contains ' A:'", () => {
  // The conservative-defer path stores free text — a Clarification empty-A would
  // corrupt on the `- Q: ... A: ...` split; the dedicated field does not.
  const item: QueueItem = {
    title: "Ambiguous",
    status: "needs-intake",
    openQuestion: "Use approach A: rules-in-code, or B: a config flag?",
    goal: "g",
    territory: ["tools/**"],
    doneWhen: "v",
    clarifications: [],
...
test("a machine-generated title containing '@' still parses (status tag is the last @)", () => {
  const item: QueueItem = {
    title: "ping user@host on failure",
    status: "needs-intake",
    origin: "auto",
