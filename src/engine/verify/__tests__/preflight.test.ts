...
test("preflight: seeds resolve against repoRoot, not surface cwd", async () => {
  // Layout: repo root contains the seed; the surface cwd is a subdir.
  // The seed path is repo-root-relative (same convention as worktree-seed.ts).
  const repo = await mkdtemp(join(tmpdir(), "preflight-"));
  await mkdir(join(repo, "tools"));
  await mkdir(join(repo, "organs"));
  await writeFile(join(repo, "organs", ".env.local"), "STUB=1\n");
  const surface: PreflightSurface = {
    name: "tools",
    cwd: join(repo, "tools"),       // subdir, not the seed's home
    repoRoot: repo,                  // seeds resolve against this
    branchPattern: "no-such-pattern-*",
...
test("preflight: regen running but producing no diff reports `no diff`, does not commit", async () => {
  ...
  const run: Runner = async (cmd) => {
    ...
    // regen change-detection now scopes the status check to the declared
    // derived paths (`git status --porcelain -- <derived...>`); empty = no diff.
    if (key === "git status --porcelain -- organs/src/generated/home-snapshot.json") return { code: 0, stdout: "", stderr: "" };
