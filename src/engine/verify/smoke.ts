// tools/orchestrator/verify/smoke.ts
...
/**
 * Run the surface's prod-render smoke on a fresh ephemeral port so parallel
 * runs never collide. Reuses the existing scripts/smoke-prod.ts (already
 * anchored on HTTP status + the exact Next client/server-exception string).
 */
export async function runSmoke(
  ...
  const r = await run(cfg.cmd, {
    cwd,
    env: { [cfg.portEnv]: String(port) },
  });
  return {
    name: "smoke",
    ok: r.code === 0,
