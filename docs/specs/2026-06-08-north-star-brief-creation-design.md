...
**CLI wiring — gate the WORK commands only:** `cmdBuild`, `cmdFix` (the audit path), `cmdDesign`/`design`, `cmdRunToGoal`, and `cmdDrain` call `loadBrief(briefPath)` + `briefGateDecision(...)` at entry; on `pass=false` they print the message and return non-zero **without doing the work**. `--skip-brief` is parsed per-run. The gate sits at the **CLI entry**, so internal drains spawned by `design`/`run-to-goal` are not double-gated. **Never gated:** `init`, `brief`, `brief write`, `audit-plan`, `check-sync`, `update` (setup/inspection commands).
