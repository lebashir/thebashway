...
| `thebashway run-to-goal [--target <id,…>] [--no-land] [--skip-brief]` | **Autonomous to a goal.** Loop build→check→repeat until the brief's success criteria are met, then stop. `--target` aims at a *slice* of the criteria (PART); omitted drives **all required** criteria (ALL). `--no-land` stages each iteration's drain at its green integration branch instead of merging to main + deploying (parity with `fix`/`build`/`drain`). Bounded by required caps (`maxIterations` default 5, a wall-clock backstop, and a build-spend ceiling) + a no-progress stall stop. Refuses to terminate on an **unconfirmed** brief; reports `goal-fully-met` only for the whole star vs `target-slice-met` for a slice; any open **milestone** parks for you instead of declaring done. |
